...
data_update_train = pd.read_csv("pima-indians-diabetes.data_update_train1.csv")
...
for i in range(len(data_update_test)):
    ...
    y.append(y01)
    list_sigmod_test.append(float(1 / (1 + math.pow(math.e, -(y[i] + B)))))
list_sigmod_test = np.array(list_sigmod_test)
ypoints = list_sigmod_test.copy()
for i in range(len(data_update_test)):
...
plt.title("Diagnose Diatabets model")
plt.xlabel("Prediction")
plt.ylabel("Stats")
plt.show()
Pie_chart = np.array([true_positive, false_negative, false_positive, true_negative])
mylabels = ["TP", "FN", "FP", "TN"]
plt.pie(Pie_chart, labels =mylabels)
plt.legend(title="Confusion  pie chart")
plt.show()
