...
def model(X_train, Y_train, learning_rate):
    ...
    while True:
        ...
        A = Sigmoid(Z)
        Cost_Function = (1 / m) * np.sum(-Y_train * np.log(A) * w1 - (1 - Y_train) * np.log(1 - A) * 4 * w1)
        print("Value of cost after Optimization :" + str(Cost_Function))
        if Cost_Function < 0.53:
            break
        dW = (1 / m) * np.dot(w1 * (4*A - Y_train - 3*A*Y_train), X_train.T)
        dB = (1 / m) * np.sum(w1 * (4*A - Y_train - 3*A*Y_train))
        dw1 = (1 / m) * np.sum(-Y_train * np.log(A)  - (1 - Y_train) * np.log(1 - A) * 4)
